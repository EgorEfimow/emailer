# AGENTS — Project Operating Manual

This document is the single source of truth for how a coding agent (opencode or
any other) must operate inside this repository. Read it before touching code.
...
- **Execution Model**: One-shot CLI binary executed by OS schedulers (cron,
  systemd). No long-running server, no HTTP API, no Webhooks.
- **No code is written in this document.** This is policy only.

See `architecture.md` for the full architectural overview and `planning.md`
for the step-by-step implementation plan.

## 2. Non-Negotiable Rules

1. Never hardcode a provider name, model name, or endpoint in code that is
   not behind the provider registry.
...
3. Implement the `Provider` interface in `internal/llm/<provider>` as a
...
3. Implement the `Channel` interface in `internal/notify/<channel>`.
...
- PRs must include:
  ...
  - Why it changed.
  - How it was tested.
  - Which document was updated (architecture, planning, AGENTS).
- No force-push to `main`.
- Squash-merge only.
- **AI Agent Prohibitions**:
  - Forbidden from adding credentials, API keys, or secrets to code,
    commits, or configuration files.
  - Forbidden from creating Pull Requests directly.
  - Forbidden from adding commit metadata (e.g., `Co-authored-by: <agent>`,
    `Generated by AI`) that implies AI authorship or counts the AI as a
    contributor.
...
- Do not assume the IMAP server allows custom keywords; check
  `PERMANENTFLAGS`.
...
- Work through `planning.md` one sub-step at a time (e.g., 2.1, then 2.2).
  Do not batch multiple sub-steps unless explicitly asked.
- **Session Start**: At the start of a new session, read `planning.md` to
  find the first unchecked `[ ]` step. Read the relevant section(s) of
  `architecture.md` for that step's phase. If the step depends on earlier
  `[x]` steps, spot-check that the code still matches what's documented
  before proceeding — do not assume a prior session's checkmark is still
  accurate.
...
- Any completed sub-step is marked `[x]` in `planning.md` in the same
  commit.
- Any new operating rule updates this file (`AGENTS.md`) in the same PR.
- Documents are code: review them, lint them, version them.
- **Drift Prevention**: The biggest structural risk is documentation
