...
// loadFixture reads a JSON fixture file from testdata/openrouter/.
...
func TestFactory_HappyPath(t *testing.T) {
	provider, err := Factory(context.Background(), "sk-or-v1-test-key", "", "openai/gpt-4o")
	if err != nil {
	...
	p := provider.(*Provider)
	if p.apiKey != "sk-or-v1-test-key" {
		t.Errorf("apiKey = %q, want %q", p.apiKey, "sk-or-v1-test-key")
	}
...
func TestFactory_EmptyAPIKey(t *testing.T) {
	_, err := Factory(context.Background(), "", "", "openai/gpt-4o")
	if err == nil {
...
func TestFactory_EmptyModel(t *testing.T) {
	_, err := Factory(context.Background(), "sk-or-v1-test-key", "", "")
	if err == nil {
...
func TestFactory_CustomEndpoint(t *testing.T) {
	provider, err := Factory(context.Background(), "sk-or-v1-test-key", "https://custom.example.com", "openai/gpt-4o")
	if err != nil {
		t.Fatalf("Factory() returned error: %v", err)
	}
...
func TestClassify_HappyPath(t *testing.T) { //nolint:gocyclo
	...
	provider, err := Factory(context.Background(), "test-key", srv.URL, "openai/gpt-4o")
	if err != nil {
		t.Fatalf("Factory() returned error: %v", err)
	}
...
func TestClassify_AuthBearerHeader(t *testing.T) {
	...
	provider, err := Factory(context.Background(), "sk-or-v1-my-secret-key", srv.URL, "openai/gpt-4o")
	if err != nil {
		t.Fatalf("Factory() returned error: %v", err)
	}
	...
	if auth != "Bearer sk-or-v1-my-secret-key" {
		t.Errorf("Authorization header = %q, want %q", auth, "Bearer sk-or-v1-my-secret-key")
	}
...
func TestProvider_Name(t *testing.T) {
	provider, err := Factory(context.Background(), "sk-or-v1-key", "", "openai/gpt-4o")
	if err != nil {
		t.Fatalf("Factory() returned error: %v", err)
	}
	if provider.Name() != "openrouter" {
